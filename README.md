# Velocity Performance Test Plugin

一個用於 Velocity Proxy 的性能測試插件，可以檢測主機規格、網路速度以及後端伺服器狀態。

## 功能特點

- ✅ **主機規格檢測** - CPU 型號、核心數、記憶體使用率、儲存空間、作業系統資訊
- ✅ **網路速度測試** - 純 Java 實現，測試下載/上傳速度、延遲
- ✅ **使用可靠的 CDN 節點** - CacheFly CDN (下載) + Fastly CDN (上傳)
- ✅ **完整測速** - 下載 50MB 檔案 + 上傳 10MB 數據
- ✅ **後端伺服器狀態** - 顯示所有後端伺服器的線上狀態和玩家數量
- ✅ **完整報告** - 一鍵查看所有性能資訊
- ✅ **無需外部工具** - 純 Java 實現，遠端主機也能使用

## 系統需求

- Velocity 3.3.0 或更高版本
- Java 21 或更高版本

## 安裝方式

### 使用 Maven 編譯

```bash
mvn clean package
```

編譯後的 JAR 檔案會在 `target/` 資料夾中。

### 安裝到 Velocity

將編譯好的 JAR 檔案放入 Velocity 伺服器的 `plugins/` 資料夾，然後重啟伺服器。

## 使用方法

### 指令列表

| 指令 | 說明 |
|------|------|
| `/vperf specs` | 顯示 Proxy 主機規格 |
| `/vperf network` | 測試網路速度 |
| `/vperf servers` | 顯示後端伺服器狀態 |
| `/vperf all` | 執行完整測試 |

### 權限設定

```
velocityperf.use - 允許使用所有 /vperf 指令
```

在 `velocity.toml` 中配置權限，或使用權限插件（如 LuckPerms）管理。

## 使用範例

### 檢查主機規格
```
/vperf specs
```

顯示資訊：
- CPU 型號和核心數
- 記憶體使用情況（JVM + 系統總記憶體）
- 儲存空間使用狀況
- 作業系統版本
- Java 版本
- 目前線上玩家數
- 後端伺服器數量

### 測試網路速度
```
/vperf network
```

測試項目：
- **下載速度** (Mbps) - 從 CacheFly CDN 下載 50MB 檔案
- **上傳速度** (Mbps) - 上傳 10MB 隨機數據到 Fastly CDN
- **延遲** (ms) - HTTP HEAD 請求測試

**測速節點：**
- 下載：CacheFly CDN (全球節點，自動路由到最近位置)
- 上傳：Fastly CDN (支援 HTTPS，全球分佈)

### 檢查後端伺服器
```
/vperf servers
```

顯示資訊：
- 伺服器名稱
- 線上/離線狀態
- 玩家數量
- 伺服器版本

### 完整測試
```
/vperf all
```

執行所有測試並顯示完整報告。

## 注意事項


### 適用場景
- 監控 Proxy 伺服器性能
- 診斷網路問題
- 檢查後端伺服器狀態
- 向玩家展示伺服器規格
- 測試伺服器上傳/下載速度

## 開發資訊

- **作者**: shi0103
- **版本**: 1.0.0